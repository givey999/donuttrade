...
import { MinecraftDepositBot } from './bot.js';
...
const DEPOSIT_MC_BOT_USERNAME = process.env.DEPOSIT_MC_BOT_USERNAME;
...
if (!MC_SERVER_HOST || !DEPOSIT_MC_BOT_USERNAME || !BOT_WEBHOOK_SECRET) {
  console.error('[Deposit Bot] Missing required env vars: MC_SERVER_HOST, DEPOSIT_MC_BOT_USERNAME, BOT_WEBHOOK_SECRET');
  process.exit(1);
}

const bot = new MinecraftDepositBot({
  server: {
    host: MC_SERVER_HOST,
  ...
  auth: {
    username: DEPOSIT_MC_BOT_USERNAME,
    type: 'microsoft',
  },
});
    ...
    for (const withdrawal of pending) {
      ...
      try {
        ...
        if (payResult.success) {
          // Server confirmed the payment — tell API
          ...
          // Server rejected the payment — fail and refund
...
bot.on('connected', (username: string) => {
  console.log(`[Deposit Bot] Connected as ${username}, monitoring deposits & processing withdrawals...`);
...
bot.on('error', (err: Error) => {
  console.error(`[Deposit Bot] Error: ${err.message}`);
});

// Start the bot
console.log(`[Deposit Bot] Starting deposit bot → ${MC_SERVER_HOST}`);
bot.connect();

// Graceful shutdown
const shutdown = () => {
  console.log('[Deposit Bot] Shutting down...');
  if (pollInterval) clearInterval(pollInterval);
  bot.disconnect();
  process.exit(0);
};
